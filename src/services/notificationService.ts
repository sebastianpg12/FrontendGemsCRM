...
export const notificationService = {
  async list(): Promise<AppNotification[]> {
    const r = await apiFetch(BASE, { headers: { ...authHeaders() } })
    if (!r.ok) throw new Error(`HTTP ${r.status}`)
    return r.json()
  },

  async unreadCount(): Promise<number> {
    const r = await apiFetch(`${BASE}/unread-count`, { headers: { ...authHeaders() } })
    if (!r.ok) return 0
