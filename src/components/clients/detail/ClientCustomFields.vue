<template>
  <div class="space-y-4">
    <div class="flex items-center gap-2 border-b border-slate-100 dark:border-[#334155] pb-3">
      <i class="fas fa-sliders-h text-[12px] text-primary-400"></i>
      <span class="text-[13px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Campos personalizados</span>
      <span v-if="(client.customFields || []).length" class="px-1.5 py-0.5 rounded-md bg-primary-50 dark:bg-primary-500/10 text-primary-600 dark:text-primary-300 text-[11px] font-black">{{ client.customFields.length }}</span>
    </div>
    ...
    <div class="bg-slate-50 dark:bg-[#0f172a] border border-slate-200 dark:border-[#334155] rounded-xl p-3 flex flex-wrap gap-2 items-end">
      <div class="flex-1 min-w-[140px]">
        <label class="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1">Campo</label>
    ...
    <div v-if="(client.customFields || []).length" class="divide-y divide-slate-100 dark:divide-[#334155] rounded-xl border border-slate-100 dark:border-[#334155] overflow-hidden">
      <div v-for="f in (client.customFields || [])" :key="f._id || f.key"
        ...
        <div class="flex items-baseline gap-3 min-w-0">
          <span class="text-[11px] font-black text-slate-400 uppercase tracking-widest shrink-0">{{ f.key }}</span>
          <span class="text-[14px] text-slate-700 dark:text-slate-200 font-semibold truncate">{{ f.value }}</span>
        </div>
    ...
    <div v-else class="text-center py-10 border border-dashed border-slate-200 dark:border-[#334155] rounded-xl">
      ...
      <p class="text-slate-400 text-[13px]">Sin campos personalizados.</p>
    </div>
  </div>
</template>
