<template>
  <div class="space-y-4">
    <div class="flex items-center gap-2 border-b border-slate-100 dark:border-[#334155] pb-3">
      <i class="fas fa-heart text-[10px] text-primary-400"></i>
      <span class="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Preferencias</span>
      <span v-if="(client.preferences || []).length" class="px-1.5 py-0.5 rounded-md bg-primary-50 dark:bg-primary-500/10 text-primary-600 dark:text-primary-300 text-[9px] font-black">{{ client.preferences.length }}</span>
    </div>
    ...
    <div v-if="(client.preferences || []).length" class="divide-y divide-slate-100 dark:divide-[#334155] rounded-xl border border-slate-100 dark:border-[#334155] overflow-hidden">
      <div v-for="p in (client.preferences || [])" :key="p._id || p.key"
        ...
        <div class="flex items-baseline gap-3 min-w-0">
          <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest shrink-0">{{ p.key }}</span>
          <span class="text-[12px] text-slate-700 dark:text-slate-200 font-semibold truncate">{{ p.value }}</span>
        </div>
    ...
    <div v-else class="text-center py-10 border border-dashed border-slate-200 dark:border-[#334155] rounded-xl">
      ...
      <p class="text-slate-400 text-[11px]">Sin preferencias registradas.</p>
    </div>
  </div>
</template>
