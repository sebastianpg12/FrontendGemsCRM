<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center gap-2 border-b border-slate-100 dark:border-[#334155] pb-3">
      <i class="fas fa-sticky-note text-[12px] text-primary-400"></i>
      <span class="text-[13px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Notas</span>
      <span v-if="sortedNotes.length" class="px-1.5 py-0.5 rounded-md bg-primary-50 dark:bg-primary-500/10 text-primary-600 dark:text-primary-300 text-[11px] font-black">{{ sortedNotes.length }}</span>
    </div>
    ...
    <div class="flex gap-2 items-start">
      <textarea v-model="newNote" rows="2"
        ...
        class="flex-1 bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-[#334155] rounded-xl px-3.5 py-2.5 text-slate-700 dark:text-slate-200 text-[14px] font-medium focus:ring-2 focus:ring-primary-400 focus:outline-none transition-all resize-none"
      ></textarea>
      <button @click="handleAddNote" :disabled="saving || !newNote.trim()"
        class="h-8 px-3.5 rounded-lg bg-primary-600 text-white text-[14px] font-black hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-colors flex items-center gap-1.5 shrink-0 mt-0.5">
        <i v-if="saving" class="fas fa-circle-notch fa-spin text-[11px]"></i>
    ...
    <div v-if="sortedNotes.length" class="space-y-2">
      <div v-for="n in sortedNotes" :key="n._id"
        ...
        <div class="flex items-start justify-between gap-2">
          <p class="text-[14px] text-slate-700 dark:text-slate-300 font-medium leading-relaxed whitespace-pre-wrap flex-1 min-w-0">{{ n.content }}</p>
          <div class="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
            <button @click="togglePinNote(n)"
              ...
              <i class="fas fa-thumbtack text-[10px]" :class="{ 'rotate-45': !n.pinned }"></i>
            </button>
        ...
        <div class="flex items-center gap-1.5 mt-1.5">
          <i v-if="n.pinned" class="fas fa-thumbtack text-amber-400 text-[9px]"></i>
          <span class="text-[12px] text-slate-400 font-medium">{{ formatDate(n.createdAt) }}</span>
        </div>
      </div>
    </div>
    ...
    <div v-else class="text-center py-10 border border-dashed border-slate-200 dark:border-[#334155] rounded-xl">
      ...
      <p class="text-slate-400 text-[13px]">Sin notas para este cliente.</p>
    </div>
      ...
      <div v-if="noteToDelete" class="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/50" @click.self="noteToDelete = null">
        <div class="bg-white dark:bg-[#1e293b] rounded-xl shadow-2xl w-full max-w-sm p-6 text-center">
          ...
          <div class="flex gap-2">
            <button @click="noteToDelete = null" class="flex-1 h-8 rounded-xl border border-slate-200 dark:border-[#334155] text-slate-600 dark:text-slate-300 text-[13px] font-bold hover:bg-slate-50 dark:hover:bg-[#334155] transition-colors">Cancelar</button>
            <button @click="confirmDeleteNote" class="flex-1 h-8 rounded-xl bg-red-500 text-white text-[13px] font-bold hover:bg-red-600 transition-colors">Eliminar</button>
          </div>
        </div>
      </div>
    </Teleport>
