  ...
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between border-b border-slate-100 dark:border-[#334155] pb-3">
      <div class="flex items-center gap-2">
        <i class="fas fa-box text-[12px] text-primary-400"></i>
        <span class="text-[13px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Servicios</span>
        <span v-if="client.services?.length" class="px-1.5 py-0.5 rounded-md bg-primary-50 dark:bg-primary-500/10 text-primary-600 dark:text-primary-300 text-[11px] font-black">{{ client.services.length }}</span>
      </div>
    </div>
    ...
    <div class="bg-slate-50 dark:bg-[#0f172a] border border-slate-200 dark:border-[#334155] rounded-xl p-3 flex flex-wrap gap-2 items-end">
      <div class="flex-1 min-w-[180px]">
        <label class="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1">Servicio</label>
      ...
      <div class="w-full sm:w-36">
        <label class="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1">Plan</label>
        <input v-model="servicePlan" placeholder="Plan Premium"
          class="w-full h-8 bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-[#334155] rounded-lg px-3 text-[14px] text-slate-700 dark:text-slate-200 font-medium focus:outline-none focus:ring-2 focus:ring-primary-400 transition-all" />
      </div>
      <div class="w-full sm:w-36">
        <label class="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1">Estado</label>
          ...
          <div class="flex-1 px-3.5 py-3 flex items-start justify-between gap-2">
            <div class="min-w-0">
              <div class="flex items-center gap-2 flex-wrap">
                <h4 class="text-[14px] font-black text-slate-800 dark:text-slate-100">{{ s.name }}</h4>
                <span :class="svcChip(s.status)" class="text-[11px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full">{{ svcLabel(s.status) }}</span>
              </div>
              <p v-if="s.plan" class="text-[12px] text-slate-400 font-medium mt-0.5">{{ s.plan }}</p>
              <p v-if="s.notes" class="text-[13px] text-slate-500 dark:text-slate-400 mt-1.5 italic">{{ s.notes }}</p>
            </div>
        ...
        <div v-else class="p-3 space-y-2">
          <div class="flex gap-2 flex-wrap">
            <input v-model="editServicePlan" placeholder="Plan"
              class="flex-1 min-w-[100px] h-8 bg-slate-50 dark:bg-[#0f172a] border border-slate-200 dark:border-[#334155] rounded-lg px-3 text-[14px] text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-primary-400" />
            <div class="w-32">
          ...
          <div class="flex gap-2 justify-end">
            <button @click="cancelEdit" class="h-7 px-3 rounded-lg bg-slate-100 dark:bg-[#334155] text-slate-500 text-[13px] font-bold hover:bg-slate-200 transition-colors">Cancelar</button>
    ...
    <div v-else class="text-center py-10 border border-dashed border-slate-200 dark:border-[#334155] rounded-xl">
      ...
      <p class="text-slate-400 text-[13px]">Sin servicios registrados.</p>
    </div>
  </div>
</template>
