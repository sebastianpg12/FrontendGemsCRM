  ...
  <div class="bg-white dark:bg-slate-900 rounded-xl overflow-hidden">
    <div class="px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
      <div>
        ...
        <p v-if="subtitle" class="text-[13px] text-slate-400 font-medium mt-0.5">{{ subtitle }}</p>
      </div>
      <button
        v-if="rows.length"
        @click="exportCsv"
        class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[12px] font-bold text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
        title="Exportar CSV"
      >
        <i class="fas fa-file-csv text-[13px]"></i>
        CSV
      </button>
    </div>
